...
use Drupal\bootstrap3\Traits\FormAutoloadFixTrait;
...
abstract class CdnProviderBase extends SettingBase {
   ...
   *
   * @param \Drupal\bootstrap3\Plugin\Provider\ProviderInterface $provider
  ...
  protected static function checkCdnExceptions(ProviderInterface $provider, $reset = TRUE) {
    ...
    if ($exceptions) {
      ...
      foreach ($exceptions as $exception) {
        watchdog_exception('bootstrap3', $exception);
      }
    }
    return !!$exceptions;
  }
   ...
   *
   * @param \Drupal\bootstrap3\Utility\Element|array $element
