...
use Drupal\bootstrap3\Plugin\AlterManager;
use Drupal\bootstrap3\Plugin\FormManager;
use Drupal\bootstrap3\Plugin\PreprocessManager;
...
class Bootstrap {
   ...
   */
  // Const PROJECT_API_SEARCH_URL = self::PROJECT_DOCUMENTATION . '/api/bootstrap3/' . self::PROJECT_BRANCH . '/search/@query';.
  ...
  public static function alter($function, &$data, &$context1 = NULL, &$context2 = NULL) {
    ...
    /** @var \Drupal\bootstrap3\Plugin\AlterManager[] $alter_managers */
    ...
    /** @var \Drupal\bootstrap3\Plugin\FormManager[] $form_managers */
    ...
    if (strpos($hook, 'form') === 0 && $context1 instanceof FormStateInterface) {
      ...
      foreach ($ids as $id) {
        /** @var \Drupal\bootstrap3\Plugin\Form\FormInterface $form */
    ...
    // Process hook alter normally.
      ...
      /** @var \Drupal\bootstrap3\Plugin\Alter\AlterInterface $class */
  ...
  public static function apiSearchUrl($query = '') {
    ...
    return new FormattableMarkup(self::PROJECT_DOCUMENTATION . '/api/bootstrap3/' . self::PROJECT_BRANCH . '/search/@query', [
      '@query' => $query,
    ]);
  }
  ...
  public static function autoloadFixInclude() {
    return static::getTheme('bootstrap3')->getPath() . '/autoload-fix.php';
  }
  ...
  public static function deprecated($caller = NULL, $show_message = NULL, ?TranslatableMarkup $message = NULL) {
    ...
    \Drupal::logger('bootstrap3')->warning('<div>@message</div><pre><code>@backtrace</code></pre>', [
      '@message' => $message,
  ...
  /**
   * Retrieves a theme instance of \Drupal\bootstrap3.
   *
   * @param string $name
  ...
  public static function glyphicon($name, array $default = []) {
    ...
    // @todo Supply a specific version to _bootstrap3_glyphicons() when Icon API
    // supports versioning.
   ...
   *   TRUE or FALSE
   *
   * @see \Drupal\bootstrap3\Bootstrap::isFront
   ...
   *   TRUE or FALSE
   *
   * @see \Drupal\bootstrap3\Bootstrap::hasIsFrontCacheContext
   * @see \Drupal\bootstrap3\Bootstrap::preprocess
   * @see https://www.drupal.org/node/2829588
   */
  ...
  public static function preprocess(array &$variables, $hook, array $info) {
    ...
    /** @var \Drupal\bootstrap3\Plugin\PreprocessManager[] $preprocess_managers */
    ...
    if (isset($info['bootstrap preprocess'])) {
      if ($preprocess_manager->hasDefinition($info['bootstrap preprocess'])) {
        ...
        /** @var \Drupal\bootstrap3\Plugin\Preprocess\PreprocessInterface $class */
