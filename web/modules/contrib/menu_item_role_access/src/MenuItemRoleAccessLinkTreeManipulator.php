...
class MenuItemRoleAccessLinkTreeManipulator extends DefaultMenuLinkTreeManipulators {
   ...
   */
  public function __construct(AccessManagerInterface $access_manager, AccountInterface $account, EntityTypeManagerInterface $entity_type_manager, ModuleHandlerInterface $module_handler = NULL, ConfigManagerInterface $config_manager, EntityRepositoryInterface $entity_repository) {
  ...
  protected function menuLinkCheckAccess(MenuLinkInterface $instance) {
    ...
    $menu_link_item = $menu_link_content_storage->load($metadata['entity_id']);
    ...
    if (!empty($allowed_roles)) {
      ...
      $users_roles = $this->account->getRoles();
    ...
    return $access_result->addCacheContexts(['user.roles']);
  }
   ...
   *   The menu link item.
   * @param bool $original_menu_link_item
   *   The original menu link item.
   *
   * @return \Drupal\menu_link_content\Entity\MenuLinkContent
   *   The menu link item whose access rules to use.
   */
  protected function getOverridingParent(MenuLinkContent $menu_link_item, $original_menu_link_item = FALSE) {
    if (empty($original_menu_link_item)) {
      $original_menu_link_item = $menu_link_item;
    }
      ...
      if ($inherit_parent_access) {
        ...
        if ($parent) {
          ...
          if ($override_children_value) {
            ...
            $menu_link_item = $this->getOverridingParent($parent, $original_menu_link_item);
          }
        }
      }
    }

    $override_children_value = $this->getOverrideChildrenValue($menu_link_item);
    if (!$override_children_value) {
      $menu_link_item = $original_menu_link_item;
    }
